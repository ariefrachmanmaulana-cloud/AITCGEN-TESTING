...
# FUNGSI GENERATOR DIPERBARUI UNTUK MENERIMA DEFAULT BARU
def generate_test_cases_with_ai(
    ...
    # PEMETAAN TAG PLATFORM SESUAI PERMINTAAN USER
    platform_mapping = {
        "WEB": "WEB",
        "Back Office": "BO",
        "Android": "AND",
    ...
    system_instruction = (
        ...
        # Perbarui contoh di bawah ini agar konsisten dengan logika platform saat ini.
        ...
        "6. **CSV Structure:** Use double quotes (\") to enclose text containing commas atau newlines (like the Test Script). The first line MUST be the header row."
        "7. **BDD Format (Test Script):** The 'Test Script (BDD)' column MUST ONLY contain the step definitions (**Given, When, Then, And, But**). DO NOT include the **'Scenario:'** keyword. Each test case must be a single block of steps, starting with **Given**, followed by **When**, and ending with **Then**. Use **And** atau **But** to chain additional steps within the single block. The structure must be: ONE 'Given', ONE 'When', ONE 'Then'."
...
if 'platform_tag' not in st.session_state:
    st.session_state['platform_tag'] = "WEB"
...
st.sidebar.markdown("""
<small>Validasi otomatis saat menekan Enter atau mengklik di luar. API key Anda tidak disimpan.</small>
...
# --- SELECT BOX UNTUK PLATFORM (DIPINDAHKAN KE BAWAH API KEY) ---
st.sidebar.markdown("---")
st.sidebar.subheader("Pilih Platform")
platform_options = ["WEB", "Back Office", "Android", "IOS", "API"]
...
# Main App
st.title("AI Test Cases Generator")
...
if not st.session_state.api_key_valid and not is_disabled_on_process_or_result:
    ...
    st.warning("⚠️ **Mohon masukkan API key Anda di sidebar dan tekan Enter** untuk mengaktifkan input dan tombol lainnya.")
# --- AKHIR INFORMASI VALIDASI API KEY ---


# --- WIDGET FILE UPLOADER ---
st.markdown("1. Unggah Dokumen Pendukung (PRD/Figma)")
...
# Prompt Input
st.markdown("---")
st.markdown("2. Ketik/Tempelkan Detail Fungsionalitas yang akan Diuji")
...
user_prompt = st.text_area(
    "",
    height=300,
    placeholder="Contoh:\nBuatkan test cases yang bisa dibuat test cases dari file ini dengan menggabungkannya. Baca semua text dari image dan juga text dari pdf nya. Test cases hanya untuk Notification for DPIA document saja serta spesifik user DPO Officer & DPO Supervisor. Jika terdapat informasi mengenai backward compatibility buatkan juga test cases-nya.\n",
...
st.markdown("3. Metadata Tambahan (Opsional)")
